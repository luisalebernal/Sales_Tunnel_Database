...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(html.H1(
            "Data Entry - Google Sheets to MySQL in the Cloud - Sales Tunnel",
    ...
    dbc.Row([
        dbc.Col([
            dbc.Accordion([
                dbc.AccordionItem([
                    html.H5([
                                "The following application allows uploading data from the online excel file to the cloud from the following link: https://docs.google.com/spreadsheets/d/1VVwtc5JN3QBvoyWRzymzEAohr9i3ak-TjlhFsGv2FNc/edit#gid=791180031 tab named 'Data Entry' to the MySQL database in the Clever Console cloud."])
    ...
    dbc.Row([
        dbc.Col([
            dbc.Accordion([
                dbc.AccordionItem([
                    html.H5([
                        'In order to enter a new table to the MySQL database in the cloud, the following requirements are needed:']),

                    html.H5([
                        "1. The columns must have the following names and in the following order: ['Item', 'Contratante', 'Unidad Estrategica', 'Tipo de Servicio', 'B. D. Inicial', 'Tipo Potencial', 'Fecha Contacto', 'Calificacion', 'Gestion / Oferta', 'Pedido', 'Ultimo Seguimiento', 'Obs. Ultimo seguimiento', 'Nombre del Proyecto', 'Estado', 'Año', 'Alcance', 'Ppto', 'Observaciones', 'Madurez', 'Seguimiento', ]."]),

                    html.H5([
                        "2. The columns 'Fecha Contacto' and 'Último Seguimiento' must have the short date format dd/mm/yyyy."]),

                    html.H5([
                        "3. The rows whose 'Item' has no information other than its number must be deleted."]),

                    html.H5([
                        "4. The column 'Ppto' must be in number format, i.e. without the $ symbol, without thousands separator and with decimal separator with dot '.'."]),
                ], title="Requirements"),
            ], start_collapsed=True, style={'textAlign': 'left', 'color': '#082255', 'font-family': "Franklin Gothic"}),
    ...
    dbc.Row([
        dbc.Tabs([
            ...
            ], label="Enter Database", label_style={'color': '#082255', 'font-family': "Franklin Gothic"}),
            ...
            dbc.Tab([
                ...
                dbc.Row([
                    dbc.Col([
                        html.Button('Change Name', id='cambiar', n_clicks=0),
                        dbc.Spinner(children=[html.Div(id='mensaje-de-exito-cambiar-nombre',
                                                       children='Press "Change" to change the MySQL database name.')],
                                    size="lg", color="primary",
                                    type="border", fullscreen=True, ),
            ...
            ], label="Change Database Name", label_style={'color': '#082255', 'font-family': "Franklin Gothic"}),
            dbc.Tab([
                dbc.Row([
                    dbc.Col(
                ...
                dbc.Row([
                    dbc.Col([
                        html.Button('Delete', id='eliminar', n_clicks=0),
            ...
            ], label="Delete Databases", label_style={'color': '#082255', 'font-family': "Franklin Gothic"}),
        ]),
    ]),
...
def ingresar_bases_de_datos(n_clicks, value_textbox):
    ...
    return 'Data successfully sent from Google sheets to Clever Console MySQL {} times'.format(
        n_clicks
    )
...
def cambiar_nombre_bases_de_datos(n_clicks, value_lista, value_nuevo_nombre):
    ...
    return 'Database name has been successfully changed from Clever Console MySQL {} times'.format(
        n_clicks
    )
...
def eliminar_bases_de_datos(n_clicks, value_lista):
    ...
    return 'Data has been successfully deleted from Clever Console MySQL {} times'.format(
        n_clicks
    )
